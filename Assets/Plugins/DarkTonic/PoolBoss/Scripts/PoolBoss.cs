...
namespace DarkTonic.PoolBoss {
	...
	public class PoolBoss : MonoBehaviour {
		...
		public static Transform Spawn(string itemName, Vector3 position, Quaternion rotation, Transform parentTransform) {
            if (!PoolItemsByName.ContainsKey(itemName)) {
                Debug.LogWarning("The Transform '" + itemName + "' has no pool in Pool Boss. It either hasn't been configured or is still preparing the pool during startup.");
                return null;
            }
        ...
        /// <summary>
        /// This method will tell you whether a Game object you specify is spawned. If false, it's inactive and can be spawned.
        /// </summary>
        /// <param name="go"></param>
        /// <returns></returns>
